...
def hasEnoughWins(
    world: World, mw: MultiWorld, state: CollectionState, player: int, *func_args
):
    yaml_win_count: int = get_option_value(
        mw,
        player,
        "total_killers_to_win_with",
    )
    possible_wins: Set[str] = world.item_name_groups["Victory Tokens"]
    current_win_count: int = 0
    for win_token in possible_wins:
        current_win_count += state.count(win_token, player)
    if current_win_count >= min(yaml_win_count, len(possible_wins)):
        return True
    return False
...
def OptAll(
    base: World, world: MultiWorld, state: CollectionState, player: int, requires: str
):
    ...
    for item in re.findall(r"\{(\w+)\(([^)]*)\)\}", requires_list):
        ...
        requires_list = requires_list.replace(
            "{" + func_name + "(" + item[1] + ")}", "{" + func_name + "(temp)}"
        )
